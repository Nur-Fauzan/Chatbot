...
public class ChatbotController : ApiController
{
    ...
    public async Task<IActionResult> GetAnswer([FromQuery] string question)
    {
        ...
        try
        {
            ...
            var apiResponse = await _httpClient.PostAsJsonAsync("http://127.0.0.1:8000/chatbot", new { text = question });

            if (apiResponse.IsSuccessStatusCode)
            {
                var apiResult = await apiResponse.Content.ReadFromJsonAsync<ChatbotApiResponse>();
                ...
                else
                {
                    ...
                    response.data = "I don't have an answer to that question yet!";
                    return new JsonResult(response);
                }
            }
            else
            {
                response.success = false;
                response.errorMessage = "Error calling Python API.";
                return new JsonResult(response);
            }
        }
        catch (Exception ex)

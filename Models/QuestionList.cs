...
public partial class chatbot {
    ...
    {
        get => HttpData.Get<QuestionList>("questionList")!;
        set => HttpData["questionList"] = value;
    }

    /// <summary>
    /// Page class for Question
    /// </summary>
    public class QuestionList : QuestionListBase
    {
        // Constructor
        public QuestionList(Controller controller) : base(controller)
        {
        }
    ...
    /// Page base class
    /// </summary>
    public class QuestionListBase : Question
    {
        // Page ID
        public string PageID = "list";

        // Project ID
        public string ProjectID = "{F74BDC2E-8C3F-45C8-A887-A125D888F9A0}";

        // Page object name
        public string PageObjName = "questionList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fQuestionlist";
        ...
        public QuestionListBase()
        {
            ...
            MultiDeleteUrl = "QuestionDelete";
            MultiUpdateUrl = "QuestionUpdate";
        ...
        // Page name
        public string PageName => "QuestionList";
        ...
        {
            ID.Visible = false;
            Question2.SetVisibility();
            Answer.Visible = false;
            Tag.SetVisibility();
        }

        // Constructor
        public QuestionListBase(Controller? controller = null): this() { // DN
            if (controller != null)
                    ...
                    if (IsModal) { // Show as modal
                        ...
                        var result = new Dictionary<string, string> { {"url", GetUrl(url)}, {"modal", "1"} }; // Assume return to modal for simplicity
                            if (!SameString(pageName, ListUrl)) { // Not List page
                                ...
                                result.Add("view", pageName == "QuestionView" ? "1" : "0"); // If View page, no primary button
                            } else { // List page
        ...
        public string GetFilterList()
        {
            ...
            filters.Merge(JObject.Parse(Question2.AdvancedSearch.ToJson())); // Field Question
            filters.Merge(JObject.Parse(Answer.AdvancedSearch.ToJson())); // Field Answer
            filters.Merge(JObject.Parse(Tag.AdvancedSearch.ToJson())); // Field Tag
        ...
        protected bool RestoreFilterList() {
            ...
            // Field Question
            if (filter?.TryGetValue("x_Question2", out sv) ?? false) {
                Question2.AdvancedSearch.SearchValue = sv;
                Question2.AdvancedSearch.SearchOperator = filter["z_Question2"];
                Question2.AdvancedSearch.SearchCondition = filter["v_Question2"];
                Question2.AdvancedSearch.SearchValue2 = filter["y_Question2"];
                Question2.AdvancedSearch.SearchOperator2 = filter["w_Question2"];
                Question2.AdvancedSearch.Save();
            }

            // Field Answer
            if (filter?.TryGetValue("x_Answer", out sv) ?? false) {
                Answer.AdvancedSearch.SearchValue = sv;
                Answer.AdvancedSearch.SearchOperator = filter["z_Answer"];
                Answer.AdvancedSearch.SearchCondition = filter["v_Answer"];
                Answer.AdvancedSearch.SearchValue2 = filter["y_Answer"];
                Answer.AdvancedSearch.SearchOperator2 = filter["w_Answer"];
                Answer.AdvancedSearch.Save();
            }

            // Field Tag
            if (filter?.TryGetValue("x_Tag", out sv) ?? false) {
                Tag.AdvancedSearch.SearchValue = sv;
                Tag.AdvancedSearch.SearchOperator = filter["z_Tag"];
                Tag.AdvancedSearch.SearchCondition = filter["v_Tag"];
                Tag.AdvancedSearch.SearchValue2 = filter["y_Tag"];
                Tag.AdvancedSearch.SearchOperator2 = filter["w_Tag"];
                Tag.AdvancedSearch.Save();
            }
        ...
        public string BasicSearchWhere(bool def = false) {
            ...
            searchFlds.Add(Question2);
            searchFlds.Add(Answer);
            searchFlds.Add(Tag);
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalView && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-view"" title=""{viewcaption}"" data-table=""Question"" data-caption=""{viewcaption}"" data-ew-action=""modal"" data-action=""view"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(ViewUrl)) + "\" data-btn=\"null\">" + Language.Phrase("ViewLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""Question"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
                    ...
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        if (action != "") {
                            ...
                            string link = disabled
                                ...
                                : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + title + "\" data-ew-action=\"submit\" form=\"fQuestionlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + " " + caption + "</button></li>";
                            if (!Empty(link)) {
                                links.Add(link);
                                if (Empty(body)) // Setup first button
                                    body = disabled
                                    ? "<div class=\"alert alert-light\">" + icon + " " + caption + "</div>"
                                    : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fQuestionlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttributes() + ">" + icon + caption + "</button>";
                            }
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""Question"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("Question")); // DN
                CreateColumnOption(option.Add("Tag")); // DN
            }
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fQuestionsrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fQuestionsrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fQuestionlist\"" + act.ToDataAttributes() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(questionList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(questionList.RowCount) + "_Question");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(questionList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && questionList.RowType == RowType.Add || IsEdit && questionList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        public async Task LoadRowValues(Dictionary<string, object>? row)
        {
            ...
            Question2.SetDbValue(row["Question"]);
            Answer.SetDbValue(row["Answer"]);
            Tag.SetDbValue(row["Tag"]);
        }
        ...
        protected Dictionary<string, object> NewRow() {
            ...
            row.Add("Question", Question2.DefaultValue ?? DbNullValue); // DN
            row.Add("Answer", Answer.DefaultValue ?? DbNullValue); // DN
            row.Add("Tag", Tag.DefaultValue ?? DbNullValue); // DN
            return row;
        }
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                // Question
                Question2.ViewValue = ConvertToString(Question2.CurrentValue); // DN
                Question2.ViewCustomAttributes = "";

                // Tag
                Tag.ViewValue = ConvertToString(Tag.CurrentValue); // DN
                Tag.ViewCustomAttributes = "";

                // Question
                Question2.HrefValue = "";
                Question2.TooltipValue = "";
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fQuestionsrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
